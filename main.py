...
async def button_handler(update: Update, context: CallbackContext):
    ...
    if query.data == "analyze":
        ...
        await query.message.reply_text("Введите данные двух объектов в формате:\nЛокация1|Площадь1|Цена1|Тип1\nЛокация2|Площадь2|Цена2|Тип2")
...
async def handle_message(update: Update, context: CallbackContext):
    try:
        ...
        analysis_result = await deepseek_analysis(tuple(data))  # Преобразуем список в кортеж
...
def calculate_investment_grade(analysis: str) -> int:
    # Пример оценки: длина анализа делится на 10
